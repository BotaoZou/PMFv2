...
class Conformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_0 = self.act1(self.bn1(self.conv1(x)))
        internal_features= [x_0]
        layer_id = [4, 8, 11]

        x_base = self.maxpool(x_0)
...
# class ConformerConstant(nn.Module):
...
#             if torch.isnan(x).any() or torch.isinf(x).any():
#                 print("Nan/Inf in convolutional branch output", i, force=True)
#             elif torch.isnan(x_t).any() or torch.isinf(x_t).any():
#                 print("Nan/Inf in transformer branch output", i, force=True)
