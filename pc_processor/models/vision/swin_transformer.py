...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                if W == 39: # for semantic-kitti-fov
                    x_out = F.interpolate(x_out, size=(H*2, W*2-1), mode='bilinear')
                else:
                    x_out = F.interpolate(x_out, size=(H*2, W*2), mode='bilinear')
                outs.append(x_out)
    ...
    def load_weights(self, model_file, is_restore=False):
...
if __name__ == "__main__":
    test_input = torch.ones(3, 3, 224, 384)
    model = swin_b()
    # model.load_weights(model_file="/home/iceory/project/pointcloudProcessor/pretrained/swin_transformer/swin_large_patch4_window12_384_22k.pth")
