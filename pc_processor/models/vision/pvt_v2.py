    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if W == 39: # for semantic-kitti-fov
                x_out = F.interpolate(x, size=(H*2, W*2-1), mode='bilinear')
            else:
                x_out = F.interpolate(x, size=(H*2, W*2), mode='bilinear')
            outs.append(x_out)
...
if __name__ == "__main__":
    model = pvt_v2_b5()
    # model.load_weights(model_file="/home/iceory/project/pointcloudProcessor/pretrained/pvt/pvt_v2_b5.pth")
    test_input = torch.ones(3, 3, 320, 320)
