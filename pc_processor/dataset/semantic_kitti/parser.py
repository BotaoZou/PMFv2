...
class SemanticKitti(object):
    ...
    def mapLidar2Camera(self, seq, pointcloud, img_h=None, img_w=None):
        if not self.has_image:
            raise ValueError("cannot mappint pointcloud with has_image=False")

        # check value of img_h and img_w
        if img_h is None:
        ...
        if img_h is not None:
            keep_idx_pts = (mapped_points[:, 1] > 0) * (mapped_points[:, 1] < img_h) * (
                    mapped_points[:, 0] > 0) * (mapped_points[:, 0] < img_w)
