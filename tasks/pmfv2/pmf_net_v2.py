...
from pc_processor.models import salsanext
...
class PMFNet(nn.Module):
    def __init__(self, pcd_channels=5, img_channels=3, nclasses=20, base_channels=32, 
                 imagenet_pretrained=True, image_backbone="resnet34", downscale=False):
        ...
        if self.downscale:
            self.downconv_lidar = nn.Conv2d(pcd_channels, pcd_channels, kernel_size=3, padding=1, stride=2, groups=pcd_channels)
            self.downconv_cam = nn.Conv2d(img_channels, img_channels, kernel_size=3, padding=1, stride=2, groups=img_channels)
    
    def forward(self, pcd_feature, img_feature):
        # 2x downscale
        if self.downscale:
            pcd_feature = self.downconv_lidar(pcd_feature)
            img_feature = self.downconv_cam(img_feature)
        ...
        # 2x upscale    
        if self.downscale:
            lidar_pred = F.interpolate(lidar_pred, scale_factor=(2, 2), mode="bilinear")
            camera_pred = F.interpolate(camera_pred, scale_factor=(2, 2), mode="bilinear")
